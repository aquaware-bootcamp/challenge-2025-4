...
public class OpenAiService {
  ...
  public Map<String,Object> chat(String prompt) throws Exception {
    // --- 🔹 MODO DEMO / SIMULADO SI NO HAY API KEY ---
    if (apiKey == null || apiKey.isBlank() || apiKey.startsWith("<") || apiKey.length() < 10) {
      System.out.println("⚠️ Ejecutando en modo simulado: no hay API key real configurada.");
      return Map.of(
        "summary", "Simulación: respuesta generada localmente.",
        "feedback", "Modo demo: sin conexión a OpenAI.",
        "input_prompt", prompt
      );
    }
